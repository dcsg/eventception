...
describe Eventception::Dispatcher do
  ...
  let(:listener_handler) {
    Eventception::EventHandler.new(listener: Eventception::Support::Listener.new, method: 'on_before')
  }
  ...
  describe '#listeners?' do
    context 'when no event_name is provided' do
      ...
      context 'and has one or more listeners associated' do
        before do
          dispatcher.add_listener(event_name: event_name, listener_handler: listener_handler)
        end
  ...
  describe '#add_listener' do
    context 'when no priority is provided' do
      ...
      it 'adds a new listener to the event_name with 0 priority' do
        ...
        expect(listeners[priority].first).to eq listener_handler
      end
    end
    ...
    context 'when priority of 10 is provided' do
      ...
      before do
        dispatcher.add_listener(event_name: event_name, listener_handler: listener_handler, priority: priority)
      end
  ...
  describe '#remove_listener' do
    let(:listener_handler2) { Eventception::EventHandler.new(listener: listener, method: 'on_after') }

    context 'when two listeners exist for the same event_name' do
      before do
        dispatcher.add_listener(event_name: event_name, listener_handler: listener_handler)
        dispatcher.add_listener(event_name: event_name, listener_handler: listener_handler2)
      end
      ...
      context 'and one of the listeners is removed' do
        it 'has only one listener' do
          dispatcher.remove_listener(event_name: event_name, listener_handler: listener_handler)
      ...
      context 'and both listeners are removed' do
        it 'has zero listeners' do
          dispatcher.remove_listener(event_name: event_name, listener_handler: listener_handler)
          dispatcher.remove_listener(event_name: event_name, listener_handler: listener_handler2)
          expect(dispatcher.listeners_for?(event_name: event_name)).to be false
  ...
  describe '#dispatch' do
    ...
    context 'when using listeners' do
      ...
      it 'executes the listener method' do
        expect(listener_handler.listener).to receive(listener_handler.method).and_call_original
